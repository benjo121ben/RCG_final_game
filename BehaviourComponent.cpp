...
void DivingBehaviour::update(FrameData& frameData) {
    if(key_pressed(GLFW_KEY_UP) && current < upperLimit){
        //gameObject->rotate(90, glm::vec3(1,0,0));
...
void MovementBehaviour::update(FrameData& frameData) {
    ...
    if(key_pressed(GLFW_KEY_D)) gameObject->rotate(-90, glm::vec3(0,1,0));
    if(key_pressed(GLFW_KEY_A)) gameObject->rotate(90, glm::vec3(0,1,0));
}
...
void ShootBehaviour::update(FrameData& frameData) {
    if(key_pressed(GLFW_KEY_SPACE) && ready) {
        ...
        g->addComponent(new BulletBehaviour(), game);
    }
    if(!key_pressed(GLFW_KEY_SPACE) && !ready) {
        ready = true;
    }
...
void BulletBehaviour::update(FrameData& frameData) {
    currentspeed = gameObject->to_world(glm::vec3(0,0,-20), 0);
}

void BulletBehaviour::update(FrameData& frameData) {
    currentspeed += gameObject->to_world(glm::vec3(0,-9.81,0.01f * timer),0) * deltaTime();
