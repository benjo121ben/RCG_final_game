...
public:
    ...
    void cleanup() {
        ...
        for(const auto& imageview : textureImageViews) {
            vkDestroyImageView(device, imageview, nullptr);
        }

        for(const VkImage& image : textureImages) {
            vkDestroyImage(device, image, nullptr);
        }
        for(const VkDeviceMemory& memory : textureImageMemories) {
...
private:
    ...
    void createDescriptorSetLayout() {
        ...
        if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &samplerDescriptorSetLayout) != VK_SUCCESS) {
            throw std::runtime_error("failed to create descriptor set layout!");
        }
    }
    ...
    void createGraphicsPipeline() {
        ...
        std::array<VkVertexInputBindingDescription, 2> descriptions = {bindingDescription, bindingDescription2};
        auto attributeDescriptions = Vertex::getAttributeDescriptions();

        vertexInputInfo.vertexBindingDescriptionCount = 2;
        vertexInputInfo.vertexAttributeDescriptionCount = static_cast<uint32_t>(attributeDescriptions.size());
        vertexInputInfo.pVertexBindingDescriptions = descriptions.data();
        ...
        std::vector<VkDescriptorSetLayout> layouts{
                uboDescriptorSetLayout, samplerDescriptorSetLayout
        };
    ...
    void createTextureImage(uint32_t texture_index) {
        ...
        createImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImages[texture_index], textureImageMemories[texture_index]);

        transitionImageLayout(textureImages[texture_index], VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
            copyBufferToImage(stagingBuffer, textureImages[texture_index], static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
        transitionImageLayout(textureImages[texture_index], VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

        vkDestroyBuffer(device, stagingBuffer, nullptr);
        vkFreeMemory(device, stagingBufferMemory, nullptr);
    }

    void createTextureImageView(uint32_t texture_index) {
        textureImageViews[texture_index] = createImageView(textureImages[texture_index], VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
    }

    void createTextureSampler(uint32_t texture_index) {
        ...
        if (vkCreateSampler(device, &samplerInfo, nullptr, &textureSamplers[texture_index]) != VK_SUCCESS) {
    ...
    void copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
        ...
        vkCmdCopyBufferToImage(commandBuffer.second, buffer, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);

        endSingleTimeCommands(commandBuffer);
    }

    void loadMesh(uint32_t model_index) {
        ...
        for (const auto& shape : shapes) {
            for (const auto& index : shape.mesh.indices) {
                ...
                if (uniqueVertices.count(vertex) == 0) {
                    uniqueVertices[vertex] = static_cast<uint32_t>(loadedMesh.vertices.size());
    ...
    std::pair<VkCommandBuffer, VkCommandBuffer> beginSingleTimeCommands() {
        ...
        VkCommandBuffer commandBufferPrim, commandBufferSec;
        vkAllocateCommandBuffers(device, &allocInfo, &commandBufferPrim);
        allocInfo.level = VK_COMMAND_BUFFER_LEVEL_SECONDARY;
        vkAllocateCommandBuffers(device, &allocInfo, &commandBufferSec);
        ...
        vkBeginCommandBuffer(commandBufferSec, &beginInfo);

        return std::make_pair(commandBufferPrim, commandBufferSec);
    }

    void endSingleTimeCommands(std::pair<VkCommandBuffer, VkCommandBuffer> commandBuffers) {
        vkEndCommandBuffer(commandBuffers.second);

        VkCommandBufferBeginInfo beginInfo{};
        beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
        beginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
        vkBeginCommandBuffer(commandBuffers.first, &beginInfo);
        vkCmdExecuteCommands(commandBuffers.first, 1, &commandBuffers.second);
        vkEndCommandBuffer(commandBuffers.first);

        VkSubmitInfo submitInfo{};
        submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
        submitInfo.commandBufferCount = 1;
        submitInfo.pCommandBuffers = &commandBuffers.first;

        vkQueueSubmit(graphicsQueue, 1, &submitInfo, VK_NULL_HANDLE);
        vkQueueWaitIdle(graphicsQueue);

        vkFreeCommandBuffers(device, commandPool, 1, &commandBuffers.first);
        vkFreeCommandBuffers(device, commandPool, 1, &commandBuffers.second);
    }
    ...
    void copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
        ...
        vkCmdCopyBuffer(commandBuffer.second, srcBuffer, dstBuffer, 1, &copyRegion);
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex, const GameObject& rootNode) {
        ...
        vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
            ...
            // viewport.height = (float) swapChainExtent.height / 5.0f;
            // vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
    ...
    void updateUniformBuffer(const GameObject& obj, Camera& camera, uint32_t currentImage) {
        ...
        //ends the timer after doing nothing, I think this is an error. It's measuring nothing.
        // This should probably be after the whole code.

        //creates empty uniform buffer
        UniformBufferObject ubo{};
        ubo.model = obj.getModelMatrix();
        //creates a view matrix for the camera
        ubo.view = glm::lookAt(camera.position + camera.offset_pos, camera.target + camera.offset_tar, camera.up);
